...
const useTeacherExams = () => {
    ...
    React.useEffect(() => {
        ...
        const load = async () => {
            ...
            setError('');
            try {
                const { getTeacherExams } = await import('../../services/apiServices.js');
                let data = [];
                if (typeof getTeacherExams === 'function') {
                    const res = await safeApiCall(getTeacherExams);
                    data = res?.data || res || [];
                }
                if (!data.length) data = fallbackExams;
...
const TeacherExams = () => {
    ...
    const handlePublish = examId => {
        setExams(prev =>
            prev.map(ex => (ex.id === examId ? { ...ex, status: 'live', startAt: new Date().toLocaleString() } : ex)),
        );
    ...
    return (
        <section>
            <header
                style={{
                    background: 'linear-gradient(135deg, rgba(20,184,166,0.16), rgba(99,102,241,0.08))',
                    padding: 20,
                    borderRadius: 18,
