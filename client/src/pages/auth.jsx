...
// If your file is actually lowercase (register.jsx), update this import accordingly.
...
const getHashMode = () =>
	typeof window === 'undefined'
		? 'login'
		: window.location.hash.replace('#', '').toLowerCase() === 'register'
			? 'register'
			: 'login';

const setHashMode = mode => {
	const next = mode === 'register' ? 'register' : 'login';
	if (typeof window !== 'undefined') {
		if (getHashMode() !== next) window.location.hash = `#${next}`;
		try {
			localStorage.setItem('authMode', next);
		} catch {}
	}
};
...
const AuthPage = () => {
	// Initialize from hash; if no hash, try localStorage, else default login
	const [mode, setMode] = useState(() => {
		const fromHash = getHashMode();
		if (typeof window === 'undefined') return fromHash;
		if (!window.location.hash) {
			try {
				const saved = (localStorage.getItem('authMode') || '').toLowerCase();
				if (saved === 'login' || saved === 'register') {
					// set hash silently on first load
	...
	// One-time migration: ?mode=... -> #mode (then strip query)
	useEffect(() => {
		if (typeof window === 'undefined') return;
		const url = new URL(window.location.href);
		const qMode = (url.searchParams.get('mode') || '').toLowerCase();
		if (qMode === 'login' || qMode === 'register') {
			// move to hash and remove the query param
			if (!window.location.hash) window.location.hash = `#${qMode}`;
	...
	// Listen to hash changes for back/forward and external changes
	useEffect(() => {
		const onHash = () => setMode(getHashMode());
