...
const normalizeMode = v => (v === 'register' ? 'register' : 'login');

const AuthPage = () => {
    const [params, setParams] = useSearchParams();

    // Initialize from URL ?mode or localStorage, fallback to 'login'
    ...
    const [mode, setModeState] = useState(initialMode); // "login" | "register"

    // Sync with URL on back/forward
    useEffect(() => {
        const urlMode = normalizeMode((params.get('mode') || 'login').toLowerCase());
        setModeState(prev => (prev === urlMode ? prev : urlMode));
    }, [params]);

    // Reflect state to URL + persist
    useEffect(() => {
        const urlMode = normalizeMode((params.get('mode') || 'login').toLowerCase());
        if (urlMode !== mode) {
    ...
    const setMode = next => setModeState(normalizeMode(next));

    // Responsive flags
    const isNarrow = useMediaQuery('(max-width: 900px)', false); // stack cols
    const isCompact = useMediaQuery('(max-width: 520px)', false); // tighter spacing
    ...
    const Switch = (
        ...
        >
            ...
            >
                <span aria-hidden="true" style={{ marginRight: 6 }}>🔑</span> Login
            </button>
            <button
                type="button"
                role="tab"
            ...
            >
                <span aria-hidden="true" style={{ marginRight: 6 }}>✨</span> Register
            </button>
        </div>
    );
                ...
                >
                    ...
                    <div style={{ ...styles.leftCol, gap: isCompact ? 10 : 12 }}>
                        ...
                        <p style={styles.subtleNote}>
                            Tip: Use the role switch inside the form to switch Student/Teacher styles.
                        </p>
                    </div>
                    ...
                    >
                        ...
                        <div style={styles.pills}>
                            {[
                                ...
                                <span key={i} style={{ ...styles.pill, color: p.c, borderColor: `${p.c}33` }}>
                                    {p.t}
                                </span>
                            ))}
                        </div>
                        ...
                        <ul style={styles.points}>
                            <li>JWT auth, role‑aware access</li>
                            <li>Rich exam and question management</li>
                            <li>Explainable scoring and review</li>
                        </ul>
                    </aside>
                </div>
            </section>
...
const styles = {
    ...
    shell: { position: 'relative', zIndex: 1, padding: 'clamp(1rem, 3vw, 2rem)', display: 'grid', placeItems: 'center' },
    grid: { display: 'grid', gap: 'clamp(1rem, 2.5vw, 2rem)', width: '100%', maxWidth: 1200, alignItems: 'stretch' },
