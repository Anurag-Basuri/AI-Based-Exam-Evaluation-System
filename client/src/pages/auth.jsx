...
const AuthPage = () => {
	...
	// Improve toggle: initialize from URL ?mode or localStorage, fallback to 'login'
	...
	const [mode, setModeState] = React.useState(initialMode); // "login" | "register"

	// Keep state in sync if user navigates back/forward (URL changes)
	React.useEffect(() => {
		const urlMode = normalizeMode((params.get('mode') || 'login').toLowerCase());
		setModeState(prev => (prev === urlMode ? prev : urlMode));
	}, [params]);

	// Reflect state to URL + persist in localStorage
	React.useEffect(() => {
		const urlMode = normalizeMode((params.get('mode') || 'login').toLowerCase());
		if (urlMode !== mode) {
	...
	return (
		<div style={styles.page}>
			...
			<div
				...
				style={{
					...styles.bgLayer,
					backgroundImage: `url(/rope-texture.png)`, // from public, no import needed
			...
			<div aria-hidden style={{ ...styles.blob, ...styles.blobC }} />
