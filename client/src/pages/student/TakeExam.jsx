...
import { StartScreen, ViolationOverlay, SubmitConfirmation } from './components/TakeExam/Overlays.jsx';
import './TakeExam.css';
...
const TakeExam = () => {
    ...
    const currentAnswer = submission?.answers?.find(a => String(a.question) === String(currentQuestion?.id));
    ...
    if (error) return <div className="exam-container" style={{ alignItems: 'center', justifyContent: 'center' }}>Error: {error}</div>;
    if (!submission) return <div className="exam-container" style={{ alignItems: 'center', justifyContent: 'center' }}>Submission not found.</div>;
    ...
    return (
        <ExamLayout>
            {violationOverlay && (
                <ViolationOverlay type={violationOverlay} onAcknowledge={handleAcknowledgeViolation} />
            )}
