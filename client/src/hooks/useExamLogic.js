...
export const useExamLogic = (submissionId) => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                const normalized = {
                    ...(subData || {}),
                    answers: Array.isArray(subData?.answers) && subData.answers.length > 0
                        ? subData.answers
    ...
    // --- Submit Logic ---
    const finalSubmit = useCallback(async (isAuto = false, reason = 'Submission confirmed.') => {
    ...
    const handleViolation = useCallback((type) => {
        ...
        apiClient.post(`/api/submissions/${submissionId}/violation`, { type }).catch(console.error);
    ...
    // --- Save Logic ---
    const handleQuickSave = useCallback(async (answersToSave, reviewState) => {
    ...
    const debouncedSave = useCallback((answers, review) => {
    ...
    const handleStartExam = async () => {
        ...
        try {
            ...
            toastError('Fullscreen denied. Please enable it for the best experience.');
        }
    };
...
const useTimer = (startedAt, duration) => {
    ...
    return useMemo(() => {
        if (!startedAt || !duration) return { remainingMs: null, remaining: { mm: '--', ss: '--' } };
