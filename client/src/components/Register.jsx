...
const Register = ({ onRegister }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (typeof onRegister === 'function') onRegister({ role, user: res?.data?.user || null });
    } catch (err) {
  ...
  const buttonShadow =
    role === 'teacher' ? '0 10px 24px rgba(249,115,22,0.28)' : '0 10px 24px rgba(79,70,229,0.25)';
      ...
      <form onSubmit={handleSubmit} style={styles.form} aria-labelledby="register-title" noValidate>
        ...
        <div style={styles.field}>
          ...
          <div style={styles.passwordWrap}>
            <input
              id="confirm"
              style={{ ...styles.input, paddingRight: 44, ...(fieldErrors.confirm ? styles.inputInvalid : {}) }}
